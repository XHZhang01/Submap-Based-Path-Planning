...
def getLidarCsvFiles(dir):
    '''Generates a list of all csv files that start with lidar'''
    lidar_files = list()
    if os.path.exists(dir):
        for path, folders, files in os.walk(dir):
            for file in files:
                if file.startswith("lidar") and os.path.splitext(file)[1] == ".csv":
                    lidar_files.append( os.path.join( path, file ) )
...
def createPointCloud2Message(timestamp_int, pos):
    timestamp_nsecs = str(timestamp_int)
    timestamp = rospy.Time( int(timestamp_nsecs[0:-9]), int(timestamp_nsecs[-9:]) )
    
    roslidar = PointCloud2()
    roslidar.header.stamp = timestamp
    
    
    return roslidar, timestamp
...
try:
    ...
    #write lidar data
    lidarfiles = getLidarCsvFiles(parsed.folder)
    for lidarfile in lidarfiles:
        topic = os.path.splitext(os.path.basename(lidarfile))[0]
       
        reader = pd.read_csv(lidarfile)
        timestamps = list(set(list(reader["TS"])))

        for timestamp in timestamps:
            data = reader[reader["TS"] == timestamp]
            pt2msg, timestamp = createPointCloud2Message(data)
            bag.write("/{0}".format(topic), imumsg, timestamp)
