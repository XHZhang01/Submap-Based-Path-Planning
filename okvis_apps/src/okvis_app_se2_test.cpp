  ...
  try {
    ...
    while (frame != config.app.max_frames) { // ToDo: check does not make sense
      ...
      if (config.app.meshing_rate > 0 && frame % config.app.meshing_rate == 0) {
        ...
        if(!config.app.slice_path.empty()){
          const std::string f = std::to_string(frame);
          map.saveFieldSlices(config.app.slice_path + "/slice_" + f + "_x_v.vtk",
                              config.app.slice_path + "/slice_" + f + "_y_v.vtk",
                              config.app.slice_path + "/slice_" + f + "_z_v.vtk",
