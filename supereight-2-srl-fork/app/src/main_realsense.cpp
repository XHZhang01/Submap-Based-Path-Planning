...
int main(int argc, char** argv)
{
    ...
    try {
        if (argc != 3) {
            std::cerr << "Usage: " << argv[0] << " YAML_FILE REALSENSE_CONFIG_FILE\n";
            return 2;
        }
        ...
        se::RealsenseReader reader(config.reader);
        /*if (reader == nullptr) {
            return EXIT_FAILURE;
        }*/
        ...
        std::string realsenseConfigFilename(argv[2]); // ToDo: adjust usage error
        srl::sensors::RealsenseParameters realsenseParams(realsenseConfigFilename);
        // Realsense RGBD sensor.
        srl::sensors::RealsenseRgbd realsense(srl::sensors::Realsense::SensorType::D455, realsenseParams);

        // Connect sensor callbacks to Realsense Reader.

        // IMU Callback, needed as otherwise other streams not started
        realsense.setImuCallback(std::bind(&se::RealsenseReader::addImuMeasurement, &reader,
                                           std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));

        // RGB Callback
        realsense.setRGBCallback(std::bind(&se::RealsenseReader::addRgbImage, &reader,
                                           std::placeholders::_1, std::placeholders::_2));
        // Depth Callback
        realsense.setAlignedDepthCallback(std::bind(&se::RealsenseReader::addDepthImage, &reader,
                                                    std::placeholders::_1, std::placeholders::_2));
        ...
        while (frame != config.app.max_frames) {
            ...
            if(read_ok == se::ReaderStatus::skip){
                frame--;
                continue;
            }
            else if (read_ok != se::ReaderStatus::ok) {
                break;
            }

            //std::vector<cv::Mat> images;
            //cv::Size res(processed_img_res.x(), processed_img_res.y());
            //images.emplace_back(res, CV_8UC4, input_depth_img.data());
            //images.emplace_back(res, CV_8UC4, input_rgba_img.data());

            //cv::imshow("depth img", images.at(0));
            //cv::waitKey();
            //cv::imshow("rgba img", images.at(1));
            //cv::waitKey();
            ...
            if ((config.app.meshing_rate > 0 && frame % config.app.meshing_rate == 0)
                ...
                if (!config.app.slice_path.empty()) {
                    map.saveFieldSlices(
                        config.app.slice_path + "/slice_x_v.vtk",
                        config.app.slice_path + "/slice_y_v.vtk",
                        config.app.slice_path + "/slice_z_v.vtk",
                        Eigen::Vector3f(0., 0., 0.), //se::math::to_translation(T_WS),
                        std::to_string(frame));
                }
                if (!config.app.structure_path.empty()) {
