...
se::ReaderType se::string_to_reader_type(const std::string& s)
{
    ...
    else if (s_lowered == "realsense"){
        return se::ReaderType::REALSENSE;
    }
    else {
...
std::string se::reader_type_to_string(se::ReaderType t)
{
    ...
    else if (t == se::ReaderType::REALSENSE){
        return "realsense";
    }
    else {
        return "unknown";
...
void se::ReaderConfig::readYaml(const std::string& filename)
{
    ...
    // ---  leica lidar parameters ---
    if(reader_type_str == "leica"){
        se::yaml::subnode_as_string(node, "leica_reader_type", leicaReaderType);
        if(leicaReaderType == "rangeImage"){
        ...
        se::yaml::subnode_as_eigen_matrix4f(node, "T_BS", T_BL);
    }
...
std::ostream& se::operator<<(std::ostream& os, const se::ReaderConfig& c)
{
    ...
    if(c.reader_type == se::ReaderType::LEICA){
        os << str_utils::value_to_pretty_str(c.leicaReaderType, "leica_reader_type") << "\n";
        ...
        os << str_utils::value_to_pretty_str(c.T_BL, "T_BS") << "\n";
    }
    return os;
}
                                      ...
                                      Eigen::Matrix4f& T_WB)
{
    ...
    status_ = mergeStatus(nextRayPose(T_WB), status_);
    if (!good()) {
        if (verbose_ >= 1) {
            std::clog << "Stopping reading due to nextPose() status: " << status_ << "\n";
        }
                                      ...
                                      std::vector<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f>>& poseBatch*/)
{
    ...
    nextFrame();
    //status_ = nextRayBatch(batch_interval, rayBatch, poseBatch);
