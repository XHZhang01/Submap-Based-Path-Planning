...
struct ReaderConfig {
    ...
    /** Starting from here is Leica Lidar specific
     * Type of Leica Reader ("rangeImage" or "ray")
     */
    std::string leicaReaderType = "";
...
class Reader {
    ...
    */
     ...
     * \return An appropriate status code.
     */
    virtual ReaderStatus nextRay(Eigen::Vector3f& ray_measurement) = 0; // ToDO: make virtual, introduce in other reader classes

    /** Read pose corresponding to current ray measurement.
     *
     * \param[out] ray_measurement The next (lidar) ray measurement.
     * \return An appropriate status code.
     */
    virtual ReaderStatus nextRayPose(Eigen::Matrix4f& T_WB) = 0; // ToDO: make virtual, introduce in other reader classes

    virtual ReaderStatus nextRayBatch(const float batch_interval,
                                      std::vector<std::pair<Eigen::Matrix4f,Eigen::Vector3f>, Eigen::aligned_allocator<std::pair<Eigen::Matrix4f,Eigen::Vector3f>>>& rayPoseBatch
                                      /*std::vector<Eigen::Vector3f, Eigen::aligned_allocator<Eigen::Vector3f>>& rayBatch,
                                      std::vector<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f>>& poseBatch*/) = 0;
