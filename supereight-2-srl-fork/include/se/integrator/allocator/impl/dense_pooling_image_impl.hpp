...
/// Leica LiDAR implementation
template<>
inline DensePoolingImage<LeicaLidar>::DensePoolingImage(const se::Image<float>& depth_map) :
    image_width_(depth_map.width()), image_height_(depth_map.height())
{
    const int image_max_dim = std::min(image_width_, image_height_);
    image_max_level_ = static_cast<int>(log2((image_max_dim - 1) / 2) + 2) - 1;
...
template<>
inline bool DensePoolingImage<LeicaLidar>::inImage(const int /* u */, const int v) const
{
    if (v >= 0 && v < static_cast<Value>(image_height_)) {
        return true;
    }
...
template<>
inline Pixel
DensePoolingImage<LeicaLidar>::poolBoundingBox(int u_min, int u_max, int v_min, int v_max) const
{
    bool u_wrap = false; ///< Pixel batch width is entirely contained within the image
...
template<>
inline Pixel DensePoolingImage<LeicaLidar>::conservativeQuery(const Eigen::Vector2i& bb_min,
                                                               const Eigen::Vector2i& bb_max) const
{
    int u_min = bb_min.x();
    int u_max = bb_max.x();
