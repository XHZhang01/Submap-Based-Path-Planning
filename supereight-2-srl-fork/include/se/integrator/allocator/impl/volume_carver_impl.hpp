...
VolumeCarver<Map<Data<se::Field::Occupancy, ColB, SemB>, se::Res::Multi, BlockSize>,
        ...
        allocated_aabb_min_(Eigen::Vector3i::Constant(octree_.getSize())),
        allocated_aabb_max_(Eigen::Vector3i::Constant(-1))
{
}
...
#pragma omp parallel for
    ...
    // Update the AABB using the allocated blocks.
    for (const se::OctantBase* node : allocation_list_.node_list) {
        if (node->isLeaf()) {
            const Eigen::Vector3i max_corner_offset =
                Eigen::Vector3i::Constant(static_cast<const NodeType*>(node)->getSize() - 1);
            allocated_aabb_min_ = allocated_aabb_min_.cwiseMin(node->getCoord());
            allocated_aabb_max_ =
                allocated_aabb_max_.cwiseMax(node->getCoord() + max_corner_offset);
        }
    }
    for (const se::OctantBase* block : allocation_list_.block_list) {
        const Eigen::Vector3i max_corner_offset =
            Eigen::Vector3i::Constant(BlockType::getSize() - 1);
        allocated_aabb_min_ = allocated_aabb_min_.cwiseMin(block->getCoord());
        allocated_aabb_max_ = allocated_aabb_max_.cwiseMax(block->getCoord() + max_corner_offset);
    }
...
VolumeCarver<Map<Data<se::Field::Occupancy, ColB, SemB>, se::Res::Multi, BlockSize>,
             SensorT>::aabbMin() const
{
    return allocated_aabb_min_;
}
...
const Eigen::Vector3i&
VolumeCarver<Map<Data<se::Field::Occupancy, ColB, SemB>, se::Res::Multi, BlockSize>,
             SensorT>::aabbMax() const
{
    return allocated_aabb_max_;
}
                                  ...
                                  se::OctantBase* octant_ptr)
{
    ...
    if (octant_depth < octree_.getBlockDepth() + 1) {
        if (num_node_corners_infront < 8) {
            ...
            if (cameraInNode(
                    octant_coord, octant_size, se::math::to_inverse_transformation(T_SW_))) {
                should_split = true;
