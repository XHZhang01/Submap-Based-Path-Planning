...
#pragma omp parallel for reduction(merge : voxel_key_set)
    ...
    for (key_t key : voxel_keys) {
        Eigen::Vector3i voxel_coord;
        scale_t _;
        keyops::decode_key(key, voxel_coord, _);
        const Eigen::Vector3i max_corner_offset =
            Eigen::Vector3i::Constant(MapT::OctreeType::block_size - 1);
        allocated_aabb_min_ = allocated_aabb_min_.cwiseMin(voxel_coord);
        allocated_aabb_max_ = allocated_aabb_max_.cwiseMax(voxel_coord + max_corner_offset);
    }
