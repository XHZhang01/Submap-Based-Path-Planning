...
Octree<DataT, ResT, BlockSize>::Octree(const int size) :
        size_(size),
        aabb_min_(Eigen::Vector3i::Constant(size)),
        aabb_max_(Eigen::Vector3i::Constant(-1))
{
...
template<typename DataT, Res ResT, int BlockSize>
const Eigen::Array3i& Octree<DataT, ResT, BlockSize>::aabbMin() const
{
    return aabb_min_;
}
...
template<typename DataT, Res ResT, int BlockSize>
const Eigen::Array3i& Octree<DataT, ResT, BlockSize>::aabbMax() const
{
    return aabb_max_;
}
...
template<typename DataT, Res ResT, int BlockSize>
void Octree<DataT, ResT, BlockSize>::aabbUpdate(const Eigen::Vector3i& voxel_coord)
{
    aabb_min_ = aabb_min_.min(voxel_coord.array());
    aabb_max_ = aabb_max_.max(voxel_coord.array());
}
