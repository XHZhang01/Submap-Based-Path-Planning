...
template<typename DataT, Res ResT>
inline Eigen::Vector3i Node<DataT, ResT>::getChildCoord(const int child_idx) const
{
    const Eigen::Vector3i child_rel_coord(
        (child_idx & 1) > 0, (child_idx & 2) > 0, (child_idx & 4) > 0);
    return getCoord() + getSize() / 2 * child_rel_coord;
}
