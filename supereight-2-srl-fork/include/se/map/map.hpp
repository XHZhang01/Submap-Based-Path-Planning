...
struct MapConfig {
    ...
    /** The desired free space resolution in metres.
     */
    float fs_res;
...
class Map<se::Data<FldT, ColB, SemB>, ResT, BlockSize> {
     ...
     * \param[in] fs_res          The desired free space resolution of the map in [meter]
    ...
    /**
     * \brief Get the desired free space resolution of the map in [meter]
     *
     * \return The resolution of the map
     */
    inline float getFreeSpaceRes() const
    {
        return free_space_resolution_;
    }
    ...
    /**
     * \brief Return a field slice mesh for the plane passing through the origin of frame O and with
     * normal the z-axis of frame O.
     */
    TriangleMesh fieldSlice(const Eigen::Matrix4f& T_WO) const;
    ...
    /** \brief The minimum coordinates in the world frame W of the AABB of the map's updated
     * regions.
     */
    Eigen::Array3f aabbMin() const;

    /** \brief The maximum coordinates in the world frame W of the AABB of the map's updated
     * regions.
     */
    Eigen::Array3f aabbMax() const;
    ...
    const float free_space_resolution_;          ///< The desired free space resolution of the map
