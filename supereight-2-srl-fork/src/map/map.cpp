...
namespace se {
MapConfig::MapConfig() :
        dim(10, 10, 3), res(0.1), fs_res(0.2), T_MW(se::math::to_transformation(Eigen::Vector3f(dim / 2)))
{
}
...
MapConfig::MapConfig(const std::string& yaml_file) : MapConfig::MapConfig()
{
    ...
    se::yaml::subnode_as_float(node, "free_space_res", fs_res);
...
std::ostream& operator<<(std::ostream& os, const MapConfig& c)
{
    ...
    os << str_utils::value_to_pretty_str(c.fs_res, "fs_res") << " m (will be rounded to next available block / node scale)\n";
